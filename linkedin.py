"""
LinkedIn Job Crawler - Main Script
...
async def run_job_search(config_path: str = "config.yaml", local_json_path: Optional[str] = None, env_path: str = '.env'):
    ...
    logger.info("🚀 Starting LinkedIn Job Crawler")
    ...
    job_filter = JobFilter(
        ...
        hours_ago=config['filters']['hours_ago'],
        exclude_sponsored=config['search']['exclude_sponsored'],
        min_employee_count=config['filters']['min_employee_count'],
        keywords=config['search'].get('keywords', []),
        nokeywords=config['filters'].get('nokeywords', [])
    )
    ...
    jobs = scraper.search_jobs(
        positions=config['search']['positions'],
    ...
    logger.info(f"Found {len(jobs)} total jobs from Apify")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='LinkedIn Job Crawler - Search for jobs and send notifications via Telegram'
    )
    parser.add_argument(
        '-c', '--config',
        type=str,
